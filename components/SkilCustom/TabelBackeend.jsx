...
function TabelBackeend() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const res = await fetch("/api/skill/backend");
  ...
  return (
    <>
      <div className="border-2 w-full lg:w-9/12 h-[52vh] rounded-lg mb-2">
        <h1 className="border-b-2 p-2 font-medium">Backend</h1>
        <div className="flex flex-col gap-2">
          {posts?.data.map((item, i) => (
            <div key={i} className="px-2 py-2">
              <h6 className="font-madium w-full text-md mb-2 flex items-center">
